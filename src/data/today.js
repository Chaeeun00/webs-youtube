import todayView01 from "../assets/img/today/todayView01.jpg";
import todayView02 from "../assets/img/today/todayView02.jpg";
import todayView03 from "../assets/img/today/todayView03.jpg";
import todayView04 from "../assets/img/today/todayView04.jpg";
import todayView05 from "../assets/img/today/todayView05.jpg";
import todayView06 from "../assets/img/today/todayView06.jpg";
import todayView07 from "../assets/img/today/todayView07.jpg";
import todayView08 from "../assets/img/today/todayView08.jpg";

export const todayText = [
    {
        img: todayView01,
        title: "🐹ちいかわ🐹 맛나 오징어, 스튜파이",
        desc : "[맛나 오징어] 자른 오징어를 먹고 있던 치이카와는 오징어옷을 입고 있는 토끼(우사기)를 발견하게 되는데... [스튜파이] 스튜파이에 빠진 토끼(우사기)를 보게 된 치이카와는 달려가는데...",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.05.11",
        page: "video/iysnbOGpvAU&t=1s"
    },{
        img: todayView02,
        title: "🐱ちいかわ🐱 혼잣말",
        desc : "뭐야 벌써 아침이네 혼잣말을 해 구워진 빵에 버터 발라 발라 비가 그치면 주변이 눅눅해 아직 축축해 이상한 발자국이 있다고 생각했는데 어젯밤에 만든 발자국 땅이 물을 잔뜩 머금어서 아직 축축해 🎵",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.05.18",
        page: "video/GKC1P0mCt4o&list=PLsmaUhkCwW3q5q46uc9So2b4D-pZqVZOV"
    },{
        img: todayView03,
        title: "🐹ちいかわ🐹 포쉐트, 쓰고 있어요",
        desc : "[포쉐트] 치이카와는 갖고 싶던 포쉐트를 사게 되는데... [쓰고 있어요] 치이카와는 신나게 길을 걷던 중 갑옷을 만나게 되는데...",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.05.25",
        page: "video/RVG38PfaBP0"
    },{
        img: todayView04,
        title: "🌰ちいかわ🌰 밤만쥬, 개복치 타기",
        desc : "[밤만쥬] 밤만쥬(쿠리만쥬)는 맛있게 밤만쥬와 구운 소세지를 먹는데... [개복치 타기] 치이카와와 토끼(우사기)는 개복치를 타고 다니다 밤만쥬(쿠리만쥬)를 보게되는데...",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.06.01",
        page: "video/5dTRQblIt-Q&t=7s"
    },{
        img: todayView05,
        title: "🐹ちいかわ🐹 아침 체조",
        desc : "이른 아침, 치이카와는 잠에서 깨는데...",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.06.08",
        page: "video/qJjz52K0ZvE"
    },{
        img: todayView06,
        title: "🐹ちいかわ🐹 도시락, 잠옷입니다",
        desc : "[도시락] 포쉐트 갑옷과 노동 갑옷, 라멘 갑옷은 서로 만든 도시락을 보여주는데... [잠옷입니다] 포쉐트 갑옷은 치이카와와 가르마(하치와레), 토끼(우사기)에게 샘플로 만든 옷을 주는데...",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.06.15",
        page: "video/McD0o5tREj0"
    },{
        img: todayView07,
        title: "🐹ちいかわ🐹 축하해",
        desc : "가르마(하치와레)는 아침 일찍 찾아온 치이카와에게 축하 선물을 받게 되는데...",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.06.22",
        page: "video/NrjwVKThap0"
    },{
        img: todayView08,
        title: "🐹ちいかわ🐹 후리카케, 갓 지은 밥",
        desc : "[후리카케] 가르마(하치와레)는 치이카와에게 그동안 모은 후리카케를 보여주는데... [갓 지은 밥] 토끼(우사기)는 치이카와와 가르마(하치와레)를 무한 쌀밥이 솟아나는 곳으로 데려가는데...",
        author : "먼가 작고 귀여운 녀석",
        channelId: "UCBTSkFNWmoOkBeZwANcbiIA",
        date: "2024.06.29",
        page: "video/fQ2GVLfmxlU"
    },
]